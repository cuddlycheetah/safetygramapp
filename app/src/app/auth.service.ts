...
export class AuthService {
  ...
  async getStatus() {
    ...
    .subscribe((res: any) => {
      ...
      this.tgAuthState.next(res._);
      if (res._.indexOf('Wait') > -1) {
    ...
    .subscribe(
      async (res: any) => {
        ...
        if (!!res) {
          ...
          if (!!res) await this.storage.set('ACCESS_TOKEN', res);
          // await this.storage.set('EXPIRES_IN', res.expires);
          this.authSubject.next(true);
          this.router.navigate(['home']);
        }
      }, async (res) => {
        console.error('oopsie happened', res.error.error);
        const alert = await this.alertCtrl.create({
          header: 'Login failed',
          message: res.error.error || 'invlid message',
          buttons: ['OK'],
        });
        alert.present();
    });
